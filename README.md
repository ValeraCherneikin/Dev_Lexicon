# Англо-русский словарь разработчиков
В этом репозитории собраны английские слова, часто встречающиеся в сфере IT. Я буду обновлять и дополнять словарь новыми словами и терминами.

# Содержание

- [Базовые термины](#Базовые-термены)
- [Программирование](#Программирование) 
- [Файлы и интерфейсы](#Файлыи-интерфейсы)
...
| -------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| above                | выше (выше в тексте, например)                               | "Refer to the **above** section for more details."           |
| activate             | активировать, включить                                       | "You need to **activate** the software before using it."     |
| authenticate         | аутентификация, проверка подлинности                         | "The system uses **authentication** to ensure that only authorized users can access the data." |
| authorize            | авторизация, разрешить                                       | "You must **authorize** this action before proceeding."      |
| automatic            | автоматический                                               | "The system performs **automatic** updates every night."     |
| below                | ниже (ниже в тексте, например)                               | "See the **below** instructions for the setup process."      |
| branch               | ветка (например, в Git-репозитории проекта)                  | "Create a new **branch** in the repository to work on the feature." |
| built-in             | встроенный                                                   | "The application comes with **built-in** tools for editing text." |
| capabilities         | возможности                                                  | "The new software has enhanced **capabilities** for data analysis." |
| change               | изменить (например, _change timezone_ — изменить временную зону) | "You can **change** the settings in the preferences menu."   |
| code, source code    | код (в смысле исходный код программы или в смысле код/номер ошибки или статуса, например, _HTTP code_ — код HTTP статуса) | "Review the **code** for errors before compiling."           |
| compile              | компилировать                                                | "You need to **compile** the code before running the application." |
| compress             | сжимать                                                      | "Use a tool to **compress** the files before sending them."  |
| create               | создавать                                                    | "You can **create** a new document by selecting 'New' from the menu." |
| current              | текущий (например, _current date_ — текущая дата)            | "The **current** date is displayed on the dashboard."        |
| data                 | данные, информация                                           | "The **data** is stored in a secure database."               |
| date                 | дата                                                         | "Select the **date** for the event from the calendar."       |
| debug                | отлаживать (программу)                                       | "Use the **debug** tools to identify and fix the issues in the code." |
| decrypt              | расшифровать                                                 | "You need to **decrypt** the message to read its contents."  |
| default              | по умолчанию                                                 | "The software uses **default** settings if no custom options are selected." |
| develop              | разрабатывать                                                | "Our team will **develop** a new feature for the application." |
| disable              | деактивировать, отключить                                    | "You can **disable** notifications in the settings menu."    |
| dynamic              | динамический                                                 | "The **dynamic** content on the website updates in real-time." |
| enable               | активировать, включить                                       | "To use this feature, you need to **enable** it in the preferences." |
| encrypt              | зашифровать                                                  | "The system will **encrypt** your data before storing it."   |
...
| open                 | открыть (например, файл)                                     | "Click the button to **open** the file."                     |
...
| английский                    | русский                                                      | Пример использования                                         |
| ----------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| boot                          | загрузка, загрузить (например, загрузить ОС Linux)           | "**Boot** the computer to start the operating system."       |
| CLI, Command Line Interface   | интерфейс командной строки                                   | "You can execute the script using the **CLI**."              |
| command                       | команда                                                      | "Enter the **command** to list all files in the directory."  |
| environment variable          | переменная среды                                             | "Set the **environment variable** to configure the application." |
| GUI, Graphical User Interface | графический пользовательский интерфейс                       | "The **GUI** allows users to interact with the software using graphical elements." |
| install, setup                | установить (например, приложение)                            | "Follow the instructions to **install** the new software."   |
| partition                     | раздел диска                                                 | "Create a new **partition** on the hard drive for better data management." |
| reboot                        | перезагрузить                                                | "After the update, you need to **reboot** the system."       |
| root                          | корень (обычно имеют в виду корневого пользователя ОС с максимальными правами) | "Log in as **root** to gain administrative privileges."      |
| shell                         | командная оболочка                                           | "Use the **shell** to execute system commands directly."     |
| shutdown                      | выключить                                                    | "Click the **shutdown** button to power off the computer."   |
| terminal                      | командный терминал                                           | "Open the **terminal** to enter your commands."              |
| uninstall                     | удалить установленное                                        | "Use the control panel to **uninstall** the program you no longer need." |
| update                        | обновить (например, приложение)                              | "Make sure to **update** the software to the latest version." |

# Интернет

| английский | русский                                                      | Пример использования                                         |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| connect    | соединяться, подключаться (например, к серверу)              | "To access the website, you need to **connect** to the internet." |
| connection | соединение                                                   | "The **connection** to the server was lost due to a network issue." |
| domain     | домен (например, yandex.ru)                                  | "Register a new **domain** for your website."                |
| download   | загружать что-то из сети/Интернет                            | "You can **download** the latest version of the software from the website." |
| gateway    | шлюз                                                         | "The **gateway** connects your local network to the wider internet." |
| hostname   | имя хоста (компьютера/сервера) в сети                        | "The **hostname** of your computer is used to identify it on the network." |
| login      | вход, войти, пройти аутентификацию (например, на сервере)    | "Enter your credentials to **login** to the system."         |
| logout     | выход, выйти (например, с сервера)                           | "Click here to **logout** from your account."                |
| network    | сеть                                                         | "Ensure your **network** connection is stable before starting the video call." |
| port       | порт, чаще всего на сервере, порт 80 — стандартный порт протокола HTTP | "Open **port 80** on the server to allow HTTP traffic."      |
| redirect   | перенаправить                                                | "The website will **redirect** you to the login page if you are not authenticated." |
| request    | запрос                                                       | "Send a **request** to the server to fetch the data."        |
| router     | роутер, маршрутизатор (от слова _route_, маршрут)            | "Connect the **router** to the modem to provide internet access to your devices." |
| server     | сервер в сети (компьютер, отвечающий на запросы по сети, или конкретная программа, запущенная на нём, обслуживающая сетевые запросы). От слова _serve_, обслуживать | "The **server** handles incoming requests and serves web pages to users." |
| upload     | выгружать что-то (например, на сервер)                       | "You need to **upload** the file to the server to share it with others." |
| hyperlink  | гиперссылка (например, ссылка на веб-странице)               | "Click the **hyperlink** to visit the related webpage."      |
